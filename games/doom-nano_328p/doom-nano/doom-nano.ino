    ...
    switch (type) {
          ...
          if (entity[i].health == 0) {
            ...
            if (entity[i].distance > ENEMY_MELEE_DIST && entity[i].distance < MAX_ENEMY_VIEW) {
              ...
              if (entity[i].state != S_MELEE) {
                ...
                // Melee attack
                player.health = max_(0, player.health - ENEMY_MELEE_DAMAGE);
                entity[i].timer = 14;
                flash_screen = 1;
                updateHud();
              }
      ...
      case E_FIREBALL: {
          if (entity[i].distance < FIREBALL_COLLIDER_DIST) {
            ...
            player.health = max_(0, player.health - ENEMY_FIREBALL_DAMAGE);
            flash_screen = 1;
            updateHud();
            removeEntity(entity[i].uid);
...
void renderGun(uint8_t gun_pos, float amount_jogging) {
  ...
  uint8_t clip_height = max_(0, min_(y + BMP_GUN_HEIGHT, RENDER_HEIGHT) - y);
