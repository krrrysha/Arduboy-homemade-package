...
struct GameSaveData {
    byte gameIDStart;           // Соответствует OFFSET_VLQP_START (1 байт)
    int svlevel;                  // Соответствует OFFSET_LEVEL (2 байта)
    unsigned long svscore;        // Соответствует OFFSET_SCORE (4 байта)
    byte svhealth;                // Соответствует OFFSET_HEALTH (1 байт)
    byte gameIDEnd;             // Соответствует OFFSET_VLQP_END (1 байт)
...
void nextLevelEnd()
{
  ...
  if (arduboy.justPressed(A_BUTTON | B_BUTTON))
  {
    ...
    saveData.gameIDStart = gameID;
    saveData.svlevel = level - 1;          
    saveData.svscore = scorePlayer;
    saveData.svhealth = coolGirl.health;
    saveData.gameIDEnd =gameID;
      ...
      EEPROM.put(OFFSET_VLQP_START, saveData);
...
void stateGameNextLevel()
{
  ...
  switch (gameOverAndStageFase) {
    case 0:
      nextLevelStart(); break;
    case 1:
      nextLevelWait(); break;
    case 2:
      nextLevelBonusCount(); break;
    case 3:
      nextLevelWait(); break;
    case 4:
      nextLevelSlideToMiddle(); break;
    case 5:
      nextLevelWait(); break;
    default:
    nextLevelEnd(); break; 
...
void stateGameContinue()
{
  ...
  EEPROM.get(OFFSET_VLQP_START, saveData);
  level =saveData.svlevel;          
  scorePlayer= saveData.svscore;
  coolGirl.health = saveData.svhealth;

  if ((saveData.gameIDStart == gameID) && (saveData.gameIDEnd == gameID))
  {
    initializePlayer(coolGirl);
