...
void setTitleFrame(TitleFrameIndex index) {
    ...
    #else
    FX::seekDataArray(TitleFrameIndexTable, idx, 0, 3 + sizeof(uint8_t));
    #endif
...
void saveSoundState() {
   ...
   //update_eeprom_byte((uint8_t*)2, arduboy.audio.enabled()); // arduboy2base::eepromAudioOnOff is protected so we just use 2 here :P
}
...
void title() { 
    ...
    switch (cookie.getMode()) {
       ...
       // if (FX::frameControl.current == 0x8C0023) {
        // Подставить адрес, который точно работает (взять из отладки SplashScreen)
       //FX::frameControl.current = 0x7EAFC; // реальный рабочий адрес
      //  }
            //setTitleFrame((TitleFrameIndex)((uint8_t)TitleFrameIndex::Intro_PoP_Frame_NC+3));
