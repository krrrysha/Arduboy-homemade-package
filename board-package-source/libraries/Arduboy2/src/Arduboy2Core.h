...
// Вариант SPI:
// Необходимо вручную перевести dip-переключателем A3 в D17
// 3v3 - (1IF)
// GND - (2IF)
// RESET - (3IF)
...
// D6 SPI1 SCK CLK (4IF)		PORT 0.2
// D5 SPI1 MOSI SDA (5IF)		PORT 0.1
// D4 DC						PORT 0.8
// D3 SPI MISO ( 6IF)			PORT 0.0
// D2 Speaker1 					PORT 0.10
// D1 (12IF)					PORT 0.6
// D0 (9IF)						PORT 0.5
 
//(без _D ) SELPI  // отключает nss IN, включает nss out к D9  PORT 1.10
//(без _D ) SELA UNO, NANO select Analo						   PORT 1.15
//(без _D ) SELB  NANO select Analog/i2c					   PORT 1.11
...
//spi
	...
	// перенёс из общего для DEV_KIT и ARDUBOY_10
	#define SPI_SS_PORT PORTB  // не очень понятно для чего этот пин определяется. и нигде не используется. Это какой-то ранее существовавший костыль https://github.com/MLXXXp/Arduboy2/issues/25
    ...
    #define CSIN_BIT 3   // -- (снимается с D9)  PORT 0.3
	#define CSOUT_BIT 14   // D9  PORT 1.14
	
	#define DC_BIT 8   // D4 PORT 0.8

	#define SPI_MOSI_BIT 1 // D5 PORT 0.1
	#define SPI_SCK_BIT 2 // D6 PORT 0.2
	#define SPI_MISO_BIT 0 // D3 PORT 0.0

	// установка CS в 1 (есть поддяжка к +)
	//#define SPI_CS_INPUT()  GPIO_0->DIRECTION_IN =  (1 << CS_BIT)
	// установка CS в 0
	//#define SPI_CS_AS_OUTPUT() GPIO_0->DIRECTION_OUT = (1 << CS_BIT) 
	//#define SPI_CS_LOW()   GPIO_0->CLEAR = (1 << CS_BIT) 
		...
		#define SSD1306_I2C_DATA 0x40
		 ...
		 #define SSD1306_I2C_CMD  0x00
		 #define SSD1306_I2C_DATA 0x40
	#endif // конец выбора конструкций, для которых опредены настройки I2C-дисплеев
#endif
...
// D7 B	
	...
	// конец настройки кнопок для ELBEARBOY-платформа
		...
		#define SPEAKER_2_BIT PORTB3
			...
			#define SPEAKER_1_BIT 10
			
			#define PIN_SPEAKER_2 7 //PORT_0_7 A3,D17 Timer16_0_out
...
// ----- DevKit pins -----
#elif defined(AB_DEVKIT) // альтернатива ARDUBOY_10 
...
#endif 
// конец настроек для ARDUBOY_10, либо для AB_DEVKIT
// --------------------
...
#define PAGE_ADDRESS_END ((HEIGHT/8)-1) & 7    // 8 pages high
	...
	void eeprom_update_byte(void*, uint8_t val);
...
class Arduboy2Core : public Arduboy2NoUSB
{
  ...
  public:
	...
	static void update_eeprom_byte(uint16_t idx, uint8_t val);
	static void write_eeprom_block(const void*, void*, size_t);
	static void read_eeprom_block(void*, const void*, size_t);
    ...
    static void inline LCDCommandMode() __attribute__((always_inline))
    {
		 ...
		 #ifdef SPIBEAR
			//bitClear(DC_PORT, DC_BIT);
			GPIO_0->CLEAR = (1 << DC_BIT) ; // command mode
		 #endif
	 #endif
    }
    /** \brief
