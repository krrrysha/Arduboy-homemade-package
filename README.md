# 1. О проекте
Данный проект является ответвлением от Arduboy2, развиваемого пользователем MrBlinky (В развиитии Arduboy участвовали многие разработчики, во время разработки изучался опыт пользователей MLXXXp и harbaum). 
...
Приблизительный диапазон значений положения джойстика для MIK32: 150...4050 
...
Приблизительный диапазон значений положения джойстика для AVR 328p: 10..680/10..1020  (переключатель уровня напряжения Joystick Shiled в положениях  3V3/5V соответственно)
...
BSP для MIK32 от ELRON позволяет использовать практически без изменений программы , разработанные в Arduino IDE для платформы AVR для плат на контроллерах MIK32 Амур.
...
Следует учитывать, что BSP от ELRON постоянно развивается и совершенствуется.
Тестирование с актуальными версиями BSP (0.5.1 и выше) еще не проводилось.

### 1) Требуется инициализация `EEPROM.begin()` при использовании функций `EEPROM.read`, `EEPROM.put` и т.п.

Необходимо добавить в функцию `void setup()` вызов `EEPROM.begin();`.
...
При вызове `EEPROM.put(offset, num)` невозможно задать `num` числом (только через переменную). При этом `offset` обычно задано через директиву `#define`.
