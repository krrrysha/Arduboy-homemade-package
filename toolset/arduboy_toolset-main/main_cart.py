...
from elboy.fx_flash_wrapper import flash_fx as elboy_flash_fx
# Подменяем оригинальную flash_fx на RISC-V версию
arduboy.serial.flash_fx = elboy_flash_fx
print(arduboy.serial.flash_fx) 

# Сохраняем оригинальную функцию "j"
_original_get_jedec_info = arduboy.serial.get_jedec_info

def custom_get_jedec_info(s_port):
    """Интерпретируем JEDEC ID как количество страниц в образе"""
    jedec_id = arduboy.serial.get_jedec_id(s_port)
    pages = (jedec_id[0] << 8) | jedec_id[1]
    capacity = pages * arduboy.constants.FX_PAGESIZE
    return arduboy.serial.JedecInfo(jedec_id, capacity, "custom-riscv")

# Подменяем функцию "j"
arduboy.serial.get_jedec_info = custom_get_jedec_info

# Сохраняем оригинальную функцию backup_fx
_original_backup_fx = arduboy.serial.backup_fx


#class SerialReadChecker:
#    """Обёртка над Serial port с проверкой длины прочитанных данных."""
...
#        if len(data) != size:
#            raise Exception(
#                f"Serial read error: requested {size} bytes, got {len(data)} bytes "
#                f"(read call #{self._read_count}). "
#                f"Port: {self._port.port}, timeout: {self._port.timeout}s. "
#                f"Data received: {data!r}"
#            )
#        return data


def custom_backup_fx(s_port, report_progress=None):
    port_name = s_port.port
    s_port.close()
    
    # Переподключаемся с нужной скоростью
    s_port = serial.Serial(port_name, 230400, timeout=2) # Увеличили timeout до 2 сек
    #s_port = serial.Serial(port_name, 115200, timeout=2) # Увеличили timeout до 2 сек
    
    # !!! КРИТИЧЕСКИ ВАЖНО: ждем инициализации USB-CDC на RISC-V !!!
    time.sleep(2.0)
    
    # !!! КРИТИЧЕСКИ ВАЖНО: очищаем буфер от мусора, накопившегося при перезагрузке МК !!!
    s_port.reset_input_buffer()
    s_port.reset_output_buffer()
    
    try:
        result = _original_backup_fx(s_port, report_progress)
    finally:
        s_port.close()
...
# Подменяем функцию
arduboy.serial.backup_fx = custom_backup_fx
