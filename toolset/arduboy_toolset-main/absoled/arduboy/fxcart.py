...
HEADER_PROGRAM_FACTOR = 256  # Multiply the flashcart slot program length by this
SAVE_ALIGNMENT = 4096
...
PROGRAM_SIZE_HEADER_INDEX = 14      # "" program size (1 byte, factor of 256 (было 128))
...
def compile_single(slot: FxParsedSlot, currentpage = 0, previouspage = 0xFFFF) -> bytearray:
    ...
    # don't flash last unused 256 bytes page
    #program_flash_size = (programsize >> 7) - 1 if program[-FLASH_PAGESIZE:] == PROGRAM_NULLPAGE else programsize >> 7
    program_flash_size = (programsize >> 8) - 1 if program[-FLASH_PAGESIZE:] == PROGRAM_NULLPAGE else programsize >> 8
    if program_flash_size > 0xFF: # Program size in 256-pages is single byte (Было half-pages)
        ...
        raise Exception(f"Somehow, program is too large for game {slot.meta.title}! Might be a problem with the binary generator! Max size: {0xFFFF} 256-pages, program was {program_flash_size}")
...
#            write_2byte_value(savepage, header, SAVEPAGE_HEADER_INDEX)
        ...
        signature = b"FXBOYDAT"
        sig_index = program.find(signature)
        
        if sig_index == -1:
            logging.warning(f"FX signature 'FXBOYDAT' NOT FOUND in binary for {slot.meta.title}! FX data will not be patched.")
        else:
            logging.debug(f"Found FX signature at 0x{sig_index:04X} for {slot.meta.title}")
            # Смещение от начала сигнатуры до полей указателей (сигнатура занимает 8 байт)
            base = sig_index + 8 
            if datasize > 0:
                # Патчим Data Key (0x9518) и Data Page
                # write_2byte_value пишет старший байт (MSB), затем младший (LSB)
                write_2byte_value(0x9518, program, base + 0)
                write_2byte_value(datapage, program, base + 2)
                # Обновляем заголовок слота на флешке
                write_2byte_value(datapage, header, DATAPAGE_HEADER_INDEX)
                write_2byte_value(datasize >> 8, header, DATA_SIZE_HEADER_INDEX)
            if savesize > 0:
                # Патчим Save Key (0x9518) и Save Page
                write_2byte_value(0x9518, program, base + 4)
                write_2byte_value(savepage, program, base + 6)
                # Обновляем заголовок слота на флешке
                write_2byte_value(savepage, header, SAVEPAGE_HEADER_INDEX)
# /модификация        
